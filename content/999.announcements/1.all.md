...
## New runner 0.11.0 is released 🎉

New runner has been [released](https://github.com/bountyhub-org/platform/releases/tag/v0.11.0)!

This version is completely compatible with the version `0.11.1`. However, the `jobengine` now contains
the new `has_diff(artifact)` expression. This has been removed previously since the artifacts were
changed from a single artifact, to multiple artifacts.

The documentation was not updated (unfortunately), causing every `has_diff` to evaluate to true.

Now, you can safely use `has_diff` in your workflow on `if` condition of the step, as well as `on.expr`.

## Development Update: Internal updates and improvements

Hello everyone! I wanted to give you a quick update on the recent internal changes and improvements we've made to enhance your experience on the platform.

For the past month, I've been focusing on improving the internal workings of the platform to ensure a smoother and more reliable experience for all users.
These changes are not immediately visible, but they are crucial for the long-term stability and performance of the platform.

To keep you informed, here are some of the key updates:

- **Improving internal APIs**: The internal APIs have been changed to be more reliable. The notify API is now stateless.
- **Complete refactor of the invoker service**: The invoker service has been completely refactored to improve reliability and performance.
- **Heavily tested the upcomming features**: There is going to be an advanced feature that will go hand-in-hand with your workflows. It is still in experimental phase, but it is coming along nicely. Stay tuned for more updates!
- **The UI refactoring**: The UI has been refactored with internal shared components which would allow faster development of new features, and more consistent user experience.

I understand that these changes may not be the best time investment for many people trying to bootstrap the company,
but this project is my passion, and I want to make sure that it is built on a solid foundation in order to ensure long-term success.

As always, you can reach out to me via the [support](https://bountyhub.org/support) page if you have any questions or concerns!

Thank you for your continued support and happy hunting! 🎯

## Fixed a bug in bhlast server responding to ipv6 requests

When bhlast DNS server answered to ipv6 requests, it would respond with A record containing ipv4 address.

This is now fixed, and the server would respond with AAAA record containing ipv6 address.

It worked previously, since most resolvers would query the A record first, and then query the AAAA record.

However, fix should speed up answers since some clients would retry the query if the wrong response is given.

Please, let me know if you have any issues!

## Applying workflow template spec to the editor has been restored

After using the workflow templates, I missed the ability to apply the template spec to the editor.
This is especially important when you start the workflow from scratch. I know you can copy the entire template,
and paste it into the editor, but that is not the best user experience.

Ideally, I would want a quick way to start from a template, and then modify it as needed.

Therefore, the ability to apply the template spec to the editor is restored. You will be promted to confirm
that you want to override the current editor content. If you confirm, the editor content will be replaced
with the template spec.

You can find this functionality in the side-bar of the workflow editor.

![Apply template spec](/workflow-tools-templates.png)

Hope this helps you get started quicker! Happy hunting! 🎯

## Sign-Out of all devices has been implemented

The platform was missing the ability to sign out of all devices. This is especially useful when you suspect that your account
is compromised, or you simply want to reset the password and sign out of all devices.

The best path forward might be that you sign-out of all devices right away, then sign-in again, and reset the password.

Luckily, this hasn't happened, but it is always better to be safe than sorry. Having said that, the feature is now implemented, and you can
find it in the [settings](https://bountyhub.org/settings) page.

Stay safe, and happy hunting! 🎯

## New runner 0.10.0 is released 🎉

New runner has been [released](https://github.com/bountyhub-org/platform/releases/tag/v0.10.0)!

The new release eliminates a big pain point running the runner: Having to run the runner from the local directory.

Now, you can run the runner from anywhere! The configuration would create a directory at `$HOME/.bountyhub/runner/<runner_name>`.

What is really important to mention is that the default name of the runner is `hostname` of the machine. So if you want to configure multiple runners
with different names, you have to specify the `--name` flag.

The API used by the runner hasn't changed, so you don't **need** to update the runner. However, always staying up to date is recommended.

## The platform has been redesigned

The platform has been redesigned from the ground up. The new design is much more user-friendly, and it is much easier to use.

If something is not working as expected, please submit a bug [here](https://github.com/bountyhub-org/platform/issues/new?template=1-platform-bug-report.yml),
or feel free to reach out directly via [Discord](https://discord.gg/EnPbMJF4dx).

While redesigning, bhlast directories are also added, so you don't need to have a flat file structure anymore.

Last but not least, the feedback form is still live. You can find it [here](https://forms.gle/2xtWVJENLAf3XJjA9).

## Major updates to the platform as promised. API changed, artifacts added, runner and bh released 🎉

Finally, after a lot of work, I'm excited to announce that the new `artifacts` field is now available in the API, along with the new `runner` and `bh` CLI versions!

The new runner `0.9.0` has been released, and it supports the new `artifacts` field. The new `bh` CLI version `0.5.0` has also been released, and it supports the new `artifacts` field.

But that is not all, the change to the API comes with some great benefits!

As previously described, the `bh job artifact download` was mostly useful when used in the runner context, since the runner
would expose environment variables for the rest of the flags, so that the command can be simplified.

Now, you don't even need that. You can download the new artifact simply by using `bh job artifact download -j <job_name> -a <artifact_name> -o <output_file>`.

I can't personally express how happy I am that the API is now much simpler, and easier to use.
The new API is much more intuitive, and it is much easier to use with the `bh` CLI.

## New API change is comming before the release of the artifacts field

Hey everyone, I would like to announce that the new API change is coming before the release of the artifacts field.

Working on the new `artifacts` field, I realized that the current API has too many subpaths, causing bad user experience.

For example, in order to get the job artifct, you have to use the following path:

```
GET /projects/{project_id}/workflows/{workflow_id}/revisions/{revision_id}/jobs/{job_id}/artifacts/{artifact_id}
```

This is too long, too deep, and too difficult to use with the `bh` CLI.

The call would have to be something like this:
...
bh job artifact download \
   ...
   -w ${{ workflow.id }} \
   -r ${{ revision.id }} \
   -j ${{ scans.subfinder[0].id }} \
   -a ${{ scans.subfinder[0].artifacts[0].id }} \
   -o output.zip
```

Since the `job_id` is already globally unique, I decided to remove the `project_id`, `workflow_id`, and `revision_id` from the path.
...
Furthermore, other routes will be trimmed down to the minimum, so they are easier to use.

For example, the new route to get the job will look like this:

```
GET /jobs/{job_id}
```

This would be a breaking change, and is the reason why the API is version as `/v0` instead of `/v1`.

It is worth noting that the new API will be released along with the new `runner`, and the new `bh` CLI version that supports the new API.

Keep an eye on the announcements, and I will let you know when the new API is released.

The platform is really shaping up, and I am excited to see how it will evolve in the future!
