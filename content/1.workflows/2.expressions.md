...
run: |
  ...
  bh job download -p ${{ job.id }} -w ${{ workflow.id }} -r ${{ revision.id }} -i ${{ scans.subfinder.filter(s, s.status == 'succeeded')[0].id}} -o first.zip
  unzip first.zip
  rm first.zip
shell: bash
```
...
run: |
  ...
  bh job download -p ${{ job.id }} -w ${{ workflow.id }} -r ${{ revision.id }} -i ${{ scans.subfinder.filter(s, s.status == 'succeeded')[1].id}} -o second.zip
  unzip second.zip
  rm second.zip
shell: bash
```
