...
#### `scans.[id].has_diff(artifact_name: string)`

This method returns true if:

- There is exactly one successful job and the `nonce` value of the artifact named `artifact_name` is not null
- There exist at least two successful jobs where the latest two successful jobs have different `nonce` values of the `artifact_name`.
...
scans:
  subfinder:
    ...
    - name: output.zip
      ...
      notify_on: diff
    steps:
      - run: subfinder -d bountyhub.org -json -o output.jsonl
        shell: bash
  httpx:
    on:
      expr: scans.subfinder.has_diff('output.zip')
    steps:
    - run: echo "Running httpx once the subfinder has a diff"
      shell: bash
```
