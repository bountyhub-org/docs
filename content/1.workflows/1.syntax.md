...
scans:
  assetfinder:
    ...
    steps:
      - run: assetfinder ${{ var.DOMAIN }} > assetfinder.out
        shell: bash
  subfinder:
    on:
...
Steps may conditionally be skipped. This is useful when you want to execute a step `if` the job dependency is done. If uses the same expression engine as `scans.on.expr`
