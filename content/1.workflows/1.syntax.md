...
scans:
  assetfinder:
    on:
      cron: 0 0 0/12 * * * # at 00:00 and at 12:00, run this scan
    uploads:
      - assetfinder.out # Specifies the file to be uploaded as a result for this scan
    steps:
      - run: assetfinder ${{ vars.DOMAIN }} > assetfinder.out
        shell: bash
  subfinder:
    on:
      cron: 0 5 0/12 * * * # At 00:05 and 12:05, run this scan
    uploads:
      - subfinder.out
    steps:
      - run: subfinder -d ${{ vars.DOMAIN }} > subfinder.out
        shell: bash
  probe-subdomains:
    on:
      expr: |
    ...
    steps:
      ...
      - run: |
          mkdir assetfinder
          cd assetfinder
          bh job download -p ${{ project.id }} -w ${{ workflow.id }} -r ${{ revision.is }} -i ${{ scans.assetfiner[0].id }} -o output.zip
          unzip output.zip
          rm output.zip
        shell: bash
      - run: 
          mkdir subfinder
          cd subfinder
          bh job download -p ${{ project.id }} -w ${{ workflow.id }} -r ${{ revision.is }} -i ${{ scans.subfiner[0].id }} -o output.zip
          unzip output.zip
          rm output.zip
        shell: bash
      - run: |
          cat assetfiner/assetfinder.out > subdomains.txt
    ...
    steps:
      ...
      - if: | 
        ...
        run: | 
          ...
          bh job download -p ${{ project.id }} -w ${{ workflow.id }} -r ${{ revision.id }} -i ${{ scans.assetfinder[0].id }} -o output.zip
      ...
      - if: |
        ...
        run: |
          ...
          bh job download -p ${{ project.id }} -w ${{ workflow.id }} -r ${{ revision.id }} -i ${{ scans.subfinder[0].id }} -o output.zip
          echo "Unzipping the result"
          unzip output.zip
          rm output.zip
          echo "Writing the result into subdomains.txt"
